...
// !!!!!!!!!!!!!!!! here LetrecExp is not defined!!!
export const isCompoundExp = (x: any): x is CompoundExp =>
  isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) || isLetExp(x) || isSetExp(x)
export const isCExp = (x: any): x is CExp => isAtomicExp(x) || isCompoundExp(x)

// ========================================================
// Parsing

export const parseL21 = (x: string): Result<Program> => bind(p(x), parseL21Program)
...
export const parseL21Program = (sexp: Sexp): Result<Program> =>
  sexp === '' || isEmpty(sexp)
    ...
    : isArray(sexp)
    ? parseL21GoodProgram(first(sexp), rest(sexp))
    : sexp

const parseL21GoodProgram = (keyword: Sexp, body: Sexp[]): Result<Program> =>
  keyword === 'L21' && !isEmpty(body)
    ? bind(mapResult(parseL21Exp, body), (exps: Exp[]) => makeOk(makeProgram(exps)))
...
const parseGoodDefine = (variable: Sexp, val: Sexp): Result<DefineExp> =>
  !isIdentifier(variable)
    ...
    : bind(parseL21CExp(val), (value: CExp) => makeOk(makeDefineExp(makeVarDecl(variable), value)))
...
export const parseL21CExp = (sexp: Sexp): Result<CExp> =>
  isEmpty(sexp)
    ...
    : isArray(sexp)
    ? parseL21CompoundCExp(first(sexp), rest(sexp))
    : isToken(sexp)
    ? parseL21Atomic(sexp)
    : sexp
...
const parseProcExp = (vars: Sexp, body: Sexp[]): Result<ProcExp> =>
  isArray(vars) && allT(isString, vars)
    ? bind(mapResult(parseL21CExp, body), (cexps: CExp[]) =>
        makeOk(makeProcExp(map(makeVarDecl, vars), cexps))
      )
    : makeFailure(`Invalid vars for ProcExp`)

const isGoodBindings = (bindings: Sexp): bindings is [string, Sexp][] =>
  isArray(bindings) && allT(isArray, bindings) && allT(isIdentifier, map(first, bindings))

...
describe('L21 Box Eval', () => {
  ...
  //   expect(
  //     bind(parseL21('(L21 (define x 1) (define y (+ x x)) (* y y))'), evalProgram)
  ...
  it('evaluates procedures', () => {
    expect(evalParse('(lambda (x) x)')).to.deep.equal(
      makeOk(makeClosure([makeVarDecl('x')], [makeVarRef('x')], theGlobalEnv))
    )
  })
  ...
  //   expect(
  //     bind(parseL21('(L21 (define square (lambda (x) (* x x))) (square 3))'), evalProgram)
  //   ).to.deep.equal(makeOk(9))
  //   expect(
  //     bind(parseL21('(L21 (define f (lambda (x) (if (> x 0) x (- 0 x)))) (f -3))'), evalProgram)
  ...
  //   expect(
  //     bind(
  //       parseL21('(L21 (define fact (lambda (n) (if (= n 0) 1 (* n (fact (- n 1)))))) (fact 3))'),
  //       evalProgram
  //     )
